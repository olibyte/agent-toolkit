...
3. Explore in subagents with `a poteto-mode subagent (read references/poteto-agent.md first)` and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
**Control skill.** Pick it by surface. Browser, Electron, and web UIs use the real UI surface. CLIs and TUIs use the real CLI surface. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
Each live lane runs on its own cloud VM at the PR head. Drive through the real UI or CLI surface (project verification skill if present).
