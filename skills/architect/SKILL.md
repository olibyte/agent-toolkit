...
Codex `$architect`. Claude Code, Cursor, and Antigravity `/architect`.
...
Naming a file isn't grounding. Produce the traced model `how` prescribes. If the design redefines ownership or layering, also run the **why** skill when that skill is in the catalog, so the rationale becomes a constraint, not a guess. Skip with reason if it is not installed.
