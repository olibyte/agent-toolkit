...
Make the factory safe and capable enough to build a real app. T13 (task toolchain) and T14 (worker hardening) are built and proven on EC2, on branch `feat/factory-setup-and-hardening`. Next is the agent proof (T15), which needs the user. The tasks are T13 to T18 in `.factory/state.json`.
...
- T13 and T14, on branch `feat/factory-setup-and-hardening`, with the design in `.factory/plan.md` under "Task setup and worker hardening":
...
1. Review and merge the T13 and T14 PR. Then close sandbox #4 and delete its branch.
2. T15, which needs the user: store `/agent-factory/anthropic-api-key` in SSM and set an Anthropic console spend limit (the AWS budget does not cover API spend). Then run `factory/examples/agent.task.json` on EC2 against the sandbox. This is the first run of Claude Code as `factory-task`. Check that the skills install lands in `/home/factory-task/.claude` and that the auto-updater's failed global writes do not break the run.
