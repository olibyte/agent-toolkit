...
Codex `$figure-it-out`. Claude Code, Cursor, and Antigravity `/figure-it-out`.
...
Log the run via the **show-me-your-work** skill when that skill is in the catalog. Skip with reason if it is not installed; keep a local TSV instead. figure-it-out's work is usually ambitious enough to commit the trail so the reviewer can read it in the PR. Commit it when confidence has to be shown. Prefer evidence produced by committed scripts. The trail plus the diff is what lets the human come back and trust the work.
